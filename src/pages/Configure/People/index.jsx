...
const People = () => {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const result = await axios(`${window.apiURL}/users`);
        const data = result.data.data.users;
        data.forEach((item) => {
          if (!item.name) item.name = "---";
          if (!item.last_name) item.last_name = "---";
          if (!item.username) item.username = "---";
          if (!item.passwordStr) item.passwordStr = "---";
          if (!item.email) item.email = "---";
          if (!item.industry) item.industry = "---";
          if (!item.industry_id) item.industry_id = "---";
